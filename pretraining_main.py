...
from dataset import AllImageDataset
import torch
import click
import os
...
@click.command()
@click.option("--image_h5_file", type=str, required=True)
def main(image_h5_file: str):
    ...
    image_dataset = AllImageDataset(image_h5_file)
    train_loader = DataLoader(
        image_dataset, batch_size=8, shuffle=False, pin_memory=True
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="train_loss",
        dirpath="checkpoints_pretrain_flow",
        filename="rvq-unet",
        save_top_k=1,
        mode="min",
    )
    ...
    torch.save(
        ...
        os.path.join(best_model_dirname, "config.json"),
    )

    best_model_dirname = "checkpoints_pretrain_flow/"
    best_model_path = os.path.join(best_model_dirname, "works-rvq-unet.ckpt")
    ...
    with torch.no_grad():
        ...
        plot_visualization(
            data[:10],
            output[:10],
            save_filename="pretrain_reconstructions.png",
            num_items=min(data.shape[0], 10),
        )
