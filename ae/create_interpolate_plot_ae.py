...
def main(interpolate_alpha):
    save_dir = Path("flow_interpolate_plot_vae")
    ...
    #     # "model_interpolate_intermediate_flow_vae/inter-rvq-unet.ckpt",
    #     n_channels=3,
    #     interpolate_alpha=interpolate_alpha,
    ...
    with torch.no_grad():
        ...
        for i in range(data_latent.shape[0]):
            ...
            extrapolated_latent = target_latent[i] + 0.25 * (
            ...
            current_save_dir = save_dir / str(interpolate_alpha) / f"interpolation_{i}"
...
if __name__ == "__main__":
    for interpolate_alpha in [0, 0.1, 0.25, 0.5, 0.75, 1]:
        main(interpolate_alpha)
